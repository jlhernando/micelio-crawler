<script lang="ts">
  ...
  async function install() {
    if (installing || !updates.status?.downloadable) return;
    installing = true;
    try {
...
{#if updates.shouldShowBanner}
  ...
  >
    ...
    <span class="flex-1 min-w-0 truncate">
      Update available
      <span class="text-fg-2 mx-1">·</span>
    ...
    <button
      ...
      onclick={install}
      disabled={installing || !updates.status?.downloadable}
      title={updates.status?.downloadable ? 'Download and install' : 'No matching binary in this release'}

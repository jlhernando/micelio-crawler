<script lang="ts">
  ...
  async function checkForUpdates() {
    try {
      ...
      toast = { message: `Update check failed: ${(err as Error).message}`, type: 'error' };
    }
  }
  ...
  async function installUpdate() {
    ...
    try {
      ...
      toast = { message: `Install failed: ${(err as Error).message}`, type: 'error' };
    } finally {
      installing = false;
    }
  }
...
<div class="max-w-4xl mx-auto space-y-4">
  ...
  {:else}
    ...
    <!-- Updates -->
    <CollapsibleSection title="Updates" open={true}>
      {#if updates.status}
        <div class="space-y-3 text-sm">
          ...
          <div class="flex flex-wrap gap-2 pt-1">
            ...
            >{updates.loading && !installing ? 'Checking…' : 'Check now'}</button>
            {#if updates.status.updateAvailable && updates.status.downloadable && !updates.status.isDevBuild}
              <button
                ...
                disabled={installing}
              >{installing ? 'Installing…' : `Install ${updates.status.latest}`}</button>
            {/if}
          </div>
